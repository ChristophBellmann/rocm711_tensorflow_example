...
PROTOBUF_SPEC="${PROTOBUF_SPEC:-protobuf<7}"
...
Options:
  ...
  --protobuf-spec <spec>    protobuf requirement (default: protobuf<7)
  --constraints <path>      Constraints output path (default: <venv>/tensorflow-rocm-constraints.txt)
  -h, --help                Show help
USAGE
}
...
"${venv_python}" -m pip install --force-reinstall "${NUMPY_SPEC}" "${PROTOBUF_SPEC}"
"${venv_python}" -m pip install --upgrade --force-reinstall "${TENSORFLOW_WHEEL}"
"${venv_python}" -m pip install --force-reinstall "${NUMPY_SPEC}" "${PROTOBUF_SPEC}"

cat >"${CONSTRAINTS_PATH}" <<EOF
# Generated by rocm711_tensorflow_example/setup_rocm_venv.sh
${NUMPY_SPEC}
${PROTOBUF_SPEC}
tensorflow @ file://${TENSORFLOW_WHEEL}
EOF

activate_script="${VENV_DIR}/bin/activate_rocm_tensorflow.sh"
python_wrapper="${VENV_DIR}/bin/python-rocm"
pip_wrapper="${VENV_DIR}/bin/pip-tf-rocm"
...
if [[ "${1:-}" == "install" ]]; then
  shift
  exec "${VIRTUAL_ENV}/bin/python" -m pip install -c "${VIRTUAL_ENV}/tensorflow-rocm-constraints.txt" "$@"
fi
